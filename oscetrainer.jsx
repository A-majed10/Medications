...
import {
  ...
  Lock, Wind, Droplets, Image as ImageIcon, Eye,
} from "lucide-react";

const MODEL = "claude-sonnet-4-6";

// Faculty passcode. Change this to whatever your department uses; only people
// who enter it can open the Faculty area to author, edit or delete stations.
const FACULTY_CODE = "osce-faculty";
...
export default function App() {
  ...
  async function loadCustom() {
    ...
    try {
      const r = await window.storage.get("hakim:cases");
      if (r && r.value) setCustomCases(JSON.parse(r.value));
    } catch (e) { /* nothing stored yet */ }
  }
  async function persist(list) {
    setCustomCases(list);
    if (typeof window === "undefined" || !window.storage) return;
    try { await window.storage.set("hakim:cases", JSON.stringify(list)); } catch (e) { /* ignore */ }
  }
  ...
  function submitGate() {
    if (gateInput.trim() === FACULTY_CODE) {
      setIsFaculty(true); setGateError(false); setGateInput(""); setView("faculty");
    } else {
      setGateError(true);
    }
  }
  ...
  function beginStation() {
    ...
    setMessages([]); setInput(""); setFeedback(null); setScoreError(false); setInvestOpen(false);
...
{"overall":<int 0-100>,"verdict":"<Clear pass|Borderline|Fail>","domains":[{"name":"<domain>","scored":<number>,"max":<int>,"items":[{"text":"<rubric item>","status":"done|partial|missed","note":"<max 12 words>"}]}],"strengths":["<short>"],"improvements":["<short>"],"expected":"<1-2 sentences: likely diagnosis or model approach>"}`;
  ...
  function saveDraft() {
    ...
    const obj = { id: editingId || ("c" + Date.now()), title, category: draft.category || "other", durationMin: Number(draft.durationMin) || 8, task: draft.task.trim() || title, brief, images, rubric };
    persist(editingId ? customCases.map((c) => c.id === editingId ? obj : c) : [...customCases, obj]);
    setView("faculty");
  }
  function deleteStation(id) { persist(customCases.filter((c) => c.id !== id)); setConfirmDelete(null); }
      ...
      <div className="mx-auto flex min-h-screen max-w-3xl flex-col bg-stone-100">
        <header className="flex items-center justify-between border-b border-stone-200 bg-white px-5 py-3.5">
          ...
          <div className="flex items-center gap-2">
            {(view === "home" || view === "faculty" || view === "editor" || view === "gate") && (
        ...
        <footer className="px-5 py-4 text-center text-xs text-stone-400">
          {t("AI standardized patient. Practice tool — not for real clinical use.", "مريض افتراضي بالذكاء الاصطناعي. أداة تدريب — ليست للاستخدام السريري.")}
        </footer>
      </div>
    ...
    return (
      <div className="px-5 py-12">
        <div className="mx-auto max-w-sm rounded-2xl border border-stone-200 bg-white p-7 text-center">
          ...
          <button onClick={submitGate} className="pine mt-4 flex w-full items-center justify-center gap-2 rounded-xl px-4 py-3 font-semibold">{t("Enter faculty area", "ادخل منطقة الهيئة")}<ChevronRight className="h-5 w-5" /></button>
    ...
    return (
      <div className="px-5 py-8">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => { setIsFaculty(false); setView("home"); }} className="text-xs font-medium text-stone-400 hover:text-stone-700">{t("Sign out", "تسجيل الخروج")}</button>
        </div>
        ...
        <p className="mt-2 text-sm text-stone-500">{t("Write your own cases, group them by specialty, attach images (ECGs, X-rays, labs), and set the mark scheme. They join the circuit and are kept on this device.", "اكتب حالاتك الخاصة، وصنّفها حسب التخصّص، وأرفِق صورًا (تخطيط قلب، أشعة، تحاليل)، وحدّد سلّم العلامات. تنضمّ إلى الحلبة وتُحفظ على هذا الجهاز.")}</p>
